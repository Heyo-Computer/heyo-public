#!/bin/sh
# Build the email-alerts Firecracker rootfs.
#
# A wrapper over `heyvm mvm build` for one reason worth a script: the Dockerfile
# lives in image/ but COPYs from guest/, so the build context has to be the
# example root. heyvm defaults the context to the Dockerfile's own directory,
# which would fail on the first COPY.
#
#   ./build-image.sh                          # -> image named email-alerts
#   IMAGE_NAME=alerts-staging ./build-image.sh
#   DNS_SERVER=10.0.0.53 ./build-image.sh     # internal resolver for the guest
#
# Env:
#   IMAGE_NAME   email-alerts   name `vm.image` in function.json must match
...
IMAGE_NAME="${IMAGE_NAME:-email-alerts}"
...
Built. Point the spec at it and register:

  sed -i 's/"image": "[^"]*"/"image": "$IMAGE_NAME"/' $HERE/function.json
  HEYOSECRET_TOKEN=… $HERE/register.sh

Check the image from inside a VM before trusting it with a page:

  heyvm create --backend-type firecracker --image $IMAGE_NAME --name preflight
