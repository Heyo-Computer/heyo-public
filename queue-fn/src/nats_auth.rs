...
//! fields; nothing in the CONNECT frame consumes them). So
//! `nats://token@host` resolves, dials, completes the TCP handshake, and *then*
//! fails with "Authorization Violation" — the failure surfaces one layer below
//! the mistake, which is why heyo's cloud service carries the same workaround
//! (`cloud/src/services/nats_auth.rs`).
//!
//! queue-fn exists to be pointed at whatever NATS is already deployed, so it
//! takes credentials the two ways operators actually supply them: embedded in
//! the URL (the NATS CLI convention, and what `CLOUD_NATS_URL` uses), or as
//! discrete env vars. This module reconciles the two into one `NatsEndpoint`.
//!
...
//!   or the metrics endpoint, so `resolve` strips userinfo and hands back
//!   sanitized servers. The credential lives in `Credential`, which has no
//!   `Debug` that reveals it.
...
///
/// All optional and all independent, so `main` can fill it from the environment
/// without deciding anything; `NatsEndpoint::resolve` is where the combinations
/// are judged.
...
impl NatsEndpoint {
    ...
    ///
    /// Env vars win over URL userinfo — an operator who sets `QFN_NATS_TOKEN`
    ...
    fn resolve(&self) -> Result<Credential, NatsAuthError> {
        ...
        if let Some(path) = &self.creds_file {
            let contents = std::fs::read_to_string(path).map_err(|e| {
                NatsAuthError::CredsFileUnreadable {
                    path: path.clone(),
                    reason: e.to_string(),
                }
            })?;
...
fn split_userinfo(entry: &str) -> Result<(String, Credential), NatsAuthError> {
    ...
    let clean = format!("{}{}{}", &entry[..after_scheme], host, &rest[authority_end..]);
...
impl fmt::Display for NatsAuthError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::EmptyUrl => write!(f, "the NATS URL is empty"),
            ...
            Self::UserWithoutPassword => write!(
                f,
                "QFN_NATS_USER is set without QFN_NATS_PASSWORD; for token auth \
                 use QFN_NATS_TOKEN instead"
            ),
            Self::PasswordWithoutUser => {
                write!(f, "QFN_NATS_PASSWORD is set without QFN_NATS_USER")
            }
...
mod tests {
    ...
    fn a_comma_separated_cluster_url_becomes_one_entry_per_server() {
        ...
        assert_eq!(e.servers, ["nats://a:4222", "nats://b:4222", "nats://c:4222"]);
    }
    ...
    fn an_unreadable_creds_file_fails_at_startup() {
        let env = EnvCredentials {
            creds_file: Some("/nonexistent/qfn.creds".into()),
            ..Default::default()
        };
        assert!(matches!(
    ...
    fn a_creds_file_is_read_at_resolve_time() {
        let path = std::env::temp_dir().join("qfn-test-nats.creds");
