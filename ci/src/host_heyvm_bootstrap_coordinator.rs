...
fn inspect(bytes:&[u8])->Result<(String,String,String)> {
    ...
    let mut outer=tar::Archive::new(std::io::Cursor::new(bytes));
    let mut found=None;
    for entry in outer.entries()? { let e=entry?; let p=e.path()?.into_owned();
        ...
        if p.to_string_lossy().ends_with("heyvm.tar.gz") { ensure!(found.is_none()&&e.header().entry_type().is_file(),"ambiguous inner heyvm archive"); let mut b=Vec::new(); e.take((LIMIT+1) as u64).read_to_end(&mut b)?; ensure!(b.len()<=LIMIT,"inner archive exceeds bound"); found=Some((p.to_string_lossy().into_owned(),b)); }
    }
...
pub async fn request(d:&Dispatcher,msg:&JobMessage,plan:&JobPlan,step:&str,alias:&str,token_expression:&str,workflow:&str,name:&str,timeout:Duration)->Result<String>{
    ...
    sqlx::query("INSERT INTO ci_host_heyvm_bootstrap(id,runner_hd_id,request,launcher_recipe,deadline,launcher_deployment_id) VALUES($1,$2,$3,$4,now()+make_interval(secs=>$5),$6)")
...
fn same_spec(actual:&Value,want:&Value)->bool{actual==want || actual.get("spec")==Some(want)}
...
async fn finish(store:&Store,id:&str,passed:bool,note:&str,result:Option<&Value>)->Result<()> {
    let mut tx=store.pool().begin().await?; let row=sqlx::query("SELECT d.run_id,d.job_id,d.step_id FROM ci_service_deployment d WHERE d.id=$1").bind(id).fetch_one(&mut *tx).await?;
    ...
    let step:String=row.get("step_id");let job:String=row.get("job_id");let run:String=row.get("run_id");
    sqlx::query("UPDATE ci_step SET status=$2,finished_at=now(),error=CASE WHEN $2='failure' THEN $3 END WHERE id=$1 AND status<>'cancelled'").bind(&step).bind(status).bind(note).execute(&mut *tx).await?;
...
async fn reconcile(d:&Dispatcher,id:&str,token:&str,configured:Option<&Target>)->Result<()> {
    ...
    let run:String=row.get("run_id");let job:String=row.get("job_id");let stopped=d.store.is_job_cancelled(&job).await?;
    ...
    ensure!(!token.is_empty(),"bootstrap token unavailable");let base=req.target.app_lb_admin_url.trim_end_matches('/');let launcher:String=row.get("launcher_deployment_id");let spec:Value=row.get("launcher_recipe");let http=client()?;
    if phase=="registering" {let r=http.get(format!("{base}/deployments/{launcher}")).bearer_auth(token).send().await?;if r.status()==reqwest::StatusCode::NOT_FOUND{body(http.post(format!("{base}/deployments")).bearer_auth(token).json(&spec).send().await?).await?;}else{ensure!(same_spec(&body(r).await?,&spec),"launcher recipe conflict");}
        ...
        sqlx::query("UPDATE ci_host_heyvm_bootstrap SET phase='delivery_ready',updated_at=now() WHERE id=$1 AND phase='registering'").bind(id).execute(d.store.pool()).await?;return Ok(())}
    let mut job_id:Option<String>=row.try_get("launcher_job_id")?;
    if phase=="delivery_ready" {sqlx::query("UPDATE ci_host_heyvm_bootstrap SET phase='armed',delivery_armed=true,updated_at=now() WHERE id=$1 AND phase='delivery_ready'").bind(id).execute(d.store.pool()).await?;
        let response=http.post(format!("{base}/deployments/{launcher}/update")).bearer_auth(token).send().await?;
        if response.status().is_success(){let j=body(response).await?;if let Some(job)=j["id"].as_str(){sqlx::query("UPDATE ci_host_heyvm_bootstrap SET launcher_job_id=$2,phase='polling',updated_at=now() WHERE id=$1 AND launcher_job_id IS NULL").bind(id).bind(job).execute(d.store.pool()).await?;}}return Ok(())}
    if phase=="armed" && job_id.is_none(){let jobs=body(http.get(format!("{base}/deployments/{launcher}/jobs")).bearer_auth(token).send().await?).await?;let matches:Vec<_>=jobs.as_array().into_iter().flatten().filter(|j|matches!(j["kind"].as_str(),Some("update"|"host-update"))).collect();
        ...
        if let Some(j)=&job_id{sqlx::query("UPDATE ci_host_heyvm_bootstrap SET launcher_job_id=$2,phase='polling',updated_at=now() WHERE id=$1 AND launcher_job_id IS NULL").bind(id).bind(j).execute(d.store.pool()).await?;}return Ok(())}
