...
if [ -b "$DATA_DEV" ]; then
    ...
    # The missing disk may contain a physical seed inhibit. The rootfs can be
    # regenerated at boot, so a rootfs marker cannot establish that it is safe
    # to initialize a replacement cluster. Keep management alive, not PG.
    echo "[init] persistent data volume $DATA_DEV missing; refusing rootfs database fallback"
    echo "HEYVM_READY"
    exec /bin/sh
fi

# A physical plan is a durable boot inhibit. Run this before any mkdir, chown,
# initdb, or configuration write can mutate an interrupted candidate. Exit 10
# means the activated standby was re-verified; 20 means keep the management
# console available but do not start (and, especially, do not initialize) PG.
PHYSICAL_ACTIVE=0
set +e
/usr/local/bin/pg-fc-physical boot-check
physical_boot_rc=$?
set -e
case "$physical_boot_rc" in
    0) ;;
    10) PHYSICAL_ACTIVE=1 ;;
    *)
        echo "[init] physical standby inhibited (boot-check rc=$physical_boot_rc)"
        echo "HEYVM_READY"
        exec /bin/sh
        ;;
esac
...
# pg_hba's `all` database keyword does NOT match a *physical* replication
# connection. Logical replication names a database, so it is already covered by
# the line above — but a future physical standby, or a `psql "replication=true"`
# diagnostic, would fail with a confusing "no pg_hba.conf entry for replication
# connection", so make the record explicit.
heal_line "$PGDATA/pg_hba.conf" "host replication all 0.0.0.0/0 trust" "host replication all 0.0.0.0/0 trust"
...
# WAL an INACTIVE replication slot may pin before Postgres invalidates it.
# This is the guard that stops a dead subscriber — or a primary whose replica
# went away for good — from filling the data disk: without it an inactive slot
# retains WAL forever and the VM dies of ENOSPC, which is a cluster-wide PANIC
# with no way back in (even booting to fix it needs disk). A slot that crosses
# this is marked `lost` and its replica has to be re-seeded, which is strictly
# the better failure: lose the replica, keep the primary.
#
# Same disk/8 budget as max_wal_size, so worst case WAL + slot is disk/4 and
# the bulk-import headroom the arithmetic above reserves is preserved.
slot_keep_mb_for() {
    wal_mb_for "$1"
}
max_wal_mb=512
...
# word onto the persistent data disk and restarting Postgres. That file is the
# only durable state involved, and it has to be, because the tuning file below
# is rewritten from scratch on EVERY boot: a wal_level set any other way would
# revert at the next stop/start, and for replication that means the slot
# survives while the WAL feeding it silently stops being logical — the worst
# possible failure, because nothing reports it until the replica is already
# behind. Regenerating the block from the marker means the running config
# always matches what the pooler last asked for, and removing the marker plus a
# restart is a complete, exact undo.
#
# Matched with `case`, never interpolated: a corrupt or hostile marker can only
# ever select one of the two known configurations.
REPL_ROLE=none
if [ -f "$WORKSPACE/heyvm-replication" ]; then
    read -r _rr < "$WORKSPACE/heyvm-replication" 2>/dev/null || _rr=""
    case "$_rr" in
        primary|replica) REPL_ROLE="$_rr" ;;
        *) echo "[init] WARNING: ignoring unrecognized $WORKSPACE/heyvm-replication" ;;
    esac
    unset _rr
fi

# Defaults reproduce the historical single-tenant configuration exactly, so a
# VM with no marker emits a byte-identical tuning file to before this existed.
wal_level_v=minimal
wal_senders_v=0
repl_slots_v=0
case "$REPL_ROLE" in
    primary)
        # `logical` is what a PUBLICATION needs. It costs more WAL volume than
        # `replica`, and both cost more than `minimal` — a permanent tax on
        # this VM's bulk-load I/O and disk high-water mark, paid for as long as
        # the pairing exists.
        wal_level_v=logical
        wal_senders_v=10
        repl_slots_v=10
        ;;
    replica)
        # A subscriber needs no WAL level of its own — it only applies changes
        # — so it keeps the cheap `minimal` profile. What it does need is slots
        # (max_replication_slots bounds replication ORIGINS on this side, not
        # senders) and apply/table-sync workers.
        repl_slots_v=8
        ;;
esac
echo "[init] replication role: $REPL_ROLE (wal_level=$wal_level_v)"
...
# Driven by the heyvm-replication marker on the data disk (see the REPL_ROLE
# block in init.sh). With no marker this is the historical single-tenant
# minimal WAL (+ compression - jsonb squeezes well), which cuts bulk-load I/O
# substantially; with one it is what a publication or a subscription needs.
wal_level = ${wal_level_v}
max_wal_senders = ${wal_senders_v}
wal_compression = lz4
...
EOF
# Everything replication-specific is appended only when there IS a marker, so a
# VM without one emits a byte-identical tuning file to before this feature
# existed — Postgres' own defaults for these (max_replication_slots = 10,
# max_slot_wal_keep_size = -1) differ from the values below, and pinning them
# fleet-wide would be a silent behaviour change for every existing database.
if [ "$REPL_ROLE" != none ]; then
    cat >> "$PGDATA/heyvm-tuning.conf" <<EOF

# Replication (role: ${REPL_ROLE}). On a primary these budget the slots the
# subscribers create; on a replica, max_replication_slots bounds replication
# ORIGINS on this side rather than senders.
max_replication_slots = ${repl_slots_v}
# The disk-safety valve: an inactive slot is invalidated at this much retained
# WAL instead of filling the data disk, which would be a cluster-wide PANIC.
max_slot_wal_keep_size = ${slot_keep_mb}MB
EOF
fi
if [ "$REPL_ROLE" = replica ]; then
    # Subscriber-side workers. max_worker_processes must cover the apply
    # worker, one table-sync worker per in-flight table AND the parallel-query
    # budget set above — or CREATE SUBSCRIPTION succeeds and then never syncs
    # anything, with only "out of background worker slots" in the server log to
    # say why.
    cat >> "$PGDATA/heyvm-tuning.conf" <<EOF
max_logical_replication_workers = 4
max_sync_workers_per_subscription = 2
max_worker_processes = $((cpus + 12))
EOF
fi
...
# Recovery-critical source minima must win over ordinary VM sizing. Move this
# include to the end on every physical boot; never rewrite auto.conf, whose
# primary_conninfo/slot were generated by pg_basebackup -R.
if [ "$PHYSICAL_ACTIVE" = 1 ]; then
    sed -i "/^include = 'pg-fc-physical.conf'$/d" "$PGDATA/postgresql.conf"
